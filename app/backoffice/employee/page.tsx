...
export default function Page() {
  ...
  const roles = ["employee", "admin", "user"];
  ...
  const fetchData = useCallback(async () => {
    try {
      const [empRes, posRes] = await Promise.all([
        axios.get(`${config.apiUrl}/api/employees/list`),
        axios.get(`${config.apiUrl}/api/positions/list`),
      ]);
  ...
  const handleSave = async () => {
    try {
      ...
      if (id == "") {
        await axios.post(`${config.apiUrl}/api/employees/create`, payload);
        Swal.fire({
        ...
        const res = await axios.put(
          `${config.apiUrl}/api/employees/updateEmployee/${id}`,
          payload
        );
  ...
  const handleAllow = async (id: string) => {
    try {
      ...
      if (button.isConfirmed) {
        await axios.put(`${config.apiUrl}/api/employees/allow/${id}`);
        Swal.fire({
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (button.isConfirmed) {
        await axios.put(`${config.apiUrl}/api/employees/remove/${id}`);
        Swal.fire({
  ...
  const handleBan = async (id: string) => {
    try {
      ...
      if (button.isConfirmed) {
        await axios.put(`${config.apiUrl}/api/employees/banned/${id}`);
        Swal.fire({
  ...
  return (
    <div className="max-w-7xl mx-auto bg-white rounded-2xl shadow-lg p-8 sm:p-10 lg:p-12">
      ...
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800">พนักงานร้าน</h1>
        <button
