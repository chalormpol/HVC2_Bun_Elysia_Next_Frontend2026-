...
export default function Page() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [empRes, posRes] = await Promise.all([
        axios.get(`${config.apiUrl}/api/admins/list`),
        axios.get(`${config.apiUrl}/api/positions/list`),
      ]);

      setEmployees(empRes.data.admins);
  ...
  const handleSave = async () => {
    try {
      ...
      if (id == "") {
        await axios.post(`${config.apiUrl}/api/admins/create`, payload);
        Swal.fire({
  ...
  const handleAllow = async (id: string) => {
    try {
      ...
      if (button.isConfirmed) {
        await axios.put(`${config.apiUrl}/api/admins/allow/${id}`);
        Swal.fire({
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (button.isConfirmed) {
        await axios.put(`${config.apiUrl}/api/admins/remove/${id}`);
        Swal.fire({
  ...
  const handleBan = async (id: string) => {
    try {
      ...
      if (button.isConfirmed) {
        await axios.put(`${config.apiUrl}/api/admins/banned/${id}`);
        Swal.fire({
